# This copies some stuff from here:
# https://github.com/niklasf/python-chess/blob/master/chess/svg.py
# I'm pretty sure MPLv.2 license used for this repo is compliant with their GPL3 .-.
...
# Jupyter wrapper
# https://ipython.readthedocs.io/en/stable/api/generated/IPython.display.html?highlight=display#IPython.display.display
...
def board(
    ...
    *, # force named params
    ...
    if board:
        ...
        for row in range(8):
            for col in range(8):
                used.add(board.get_piece(row, col))
        for piece in used:
            if piece != Piece.AIR:
                defs.append(ET.fromstring(PIECES[piece.symbol]))
    
    defs.append(ET.fromstring(XX))

    # Render coordinates.
    if coordinates:
        margin_color, margin_opacity = _select_color(colours, "margin")
        ...
        coord_color, coord_opacity = _select_color(colours, "coord")
        for file_index, file_name in enumerate([chr(i) for i in range(ord("a"), ord("i"))]):
        ...
        for rank_index, rank_name in enumerate([str(i) for i in range(1,9)]):
    ...
    for row in range(8):
        for col in range(8):
            x = (col if orientation else 7 - col) * SQUARE_SIZE + margin
            y = (7 - row if orientation else row) * SQUARE_SIZE + margin
